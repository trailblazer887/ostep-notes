...
本章将对I/O设备的作基本介绍 \\
简单来说(并不实际)，I/O devices包括\textbf{interface}(status, command, data register)和\textbf{internals}(一些硬件，如micro-controller(CPU)等)两个部分。\\
\textbf{device driver}就像OS的"翻译官"，将具体不同的硬件操作抽象化提供给OS，使得OS能够调用统一的接口来操作I/O。
